...
parser.add_argument('--config', type=str, default='config_files/def_rdkit_config.yaml', 
   help='YAML file defineing experiment documentation (see README)')
#
...
#x_summary = [str(obj) for obj in [np.shape(x), np.sum(x, axis=axis)]]
y_summary = [str(obj) for obj in [np.shape(y), np.sum(y, axis=axis)]]
#logger.info('X shape:%s  X sum:%s'%tuple(x_summary))
logger.info('Y shape:%s  Y sum:%s'%tuple(y_summary))
...
logger.info('Saving model weights and model')
gz_pickle(args.output_core+'_train_weights.pkz', model.get_weights())
model.save(args.output_core+'_train_model')
